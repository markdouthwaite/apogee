...
def factor_marginalise(a, v):
    ...
    assignments = ap.cartesian_product(*[np.arange(x) for x in card])  # compute assignments only once.
    f_idx = ap.array_index(a.assignments[:, f_map], assignments)  # extract indices to be combined.
